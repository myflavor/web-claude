...
type Config struct {
	...
	// Empty = no root restriction beyond absolute paths you open (not used; we always set a root).
	ProjectsRoot string
...
func Load() (*Config, error) {
	...
	mode := strings.ToLower(env("RUN_MODE", "auto"))
	// auto: if PROJECTS_ROOT or HOME_DIR looks like container paths, docker-ish; else native.
	if mode == "auto" {
		if os.Getenv("PROJECTS_ROOT") != "" || os.Getenv("HOME_DIR") != "" || os.Getenv("CLAUDE_HOME") != "" {
			// explicit paths → keep as provided below
			mode = "custom"
	...
	case "docker":
		...
		cfg.ProjectsRoot = env("PROJECTS_ROOT", "/data/projects")
		cfg.ClaudeHome = env("CLAUDE_HOME", env("HOME_DIR", "/data/home"))
		cfg.InheritUserEnv = false
	default: // native / wsl / host / custom / explicit env
		home, _ := os.UserHomeDir()
		cfg.ProjectsRoot = env("PROJECTS_ROOT", defaultProjectsRoot(home))
		// Empty ClaudeHome → do not override HOME (use ~/.claude as-is).
		cfg.ClaudeHome = env("CLAUDE_HOME", env("HOME_DIR", ""))
		cfg.InheritUserEnv = cfg.ClaudeHome == ""
	}

	// Optional explicit config dir (advanced).
	cfg.ClaudeConfigDir = env("CLAUDE_CONFIG_DIR", "")
	...
	if cfg.AuthToken == "" {
		return nil, fmt.Errorf("AUTH_TOKEN is required (web login password)")
	}
	if cfg.ProjectsRoot == "" {
		return nil, fmt.Errorf("PROJECTS_ROOT is required")
	}
...
func defaultProjectsRoot(home string) string {
	// Prefer ~/projects if exists, else current working directory.
	if home != "" {
		cand := filepath.Join(home, "projects")
